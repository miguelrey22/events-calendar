    ...
    def get_airtable_data(self, table_name, max_records=None, fields=None):
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"❌ Error HTTP {e.response.status_code} en {table_name}: {e.response.text[:200]}")
                if attempt >= max_attempts:
                    raise Exception(f"Error HTTP {e.response.status_code} en {table_name}: {e.response.text[:200]}")
                time.sleep(2)
    ...
    def _process_employees(self, employees_data):
        ...
        for record in employees_data:
            ...
            employees_list.append({
                ...
                'Email': fields.get('Email address'),  # Nombre correcto
                'Position': fields.get('Job Role'),    # Nombre correcto
